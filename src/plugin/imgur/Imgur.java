	...
	public void onMessage(Message in_message) throws Exception {
		...
		if (m.find()) {
			...
			while ((line = in.readLine()) != null) {
				...
				m = Pattern
						.compile(
								"<div class=\"title negative \" title=\"([\\d,]*) dislikes\" style=\"width: [\\d.]*%\"></div>",
								Pattern.CASE_INSENSITIVE | Pattern.DOTALL)
						.matcher(line);
				if (m.find())
					dislikes = m.group(1);
				...
				m = Pattern
						.compile(
								"<div class=\"title positive \" title=\"([\\d,]*) likes\" style=\"width: [\\d.]*%\"></div>",
								Pattern.CASE_INSENSITIVE | Pattern.DOTALL)
						.matcher(line);
				if (m.find())
					likes = m.group(1);
				...
				m = Pattern
						.compile(
								"<span id=\"stats-bandwidth\" class=\"stat\">([\\d\\w\\s.]*)</span> bandwidth</span>",
								Pattern.CASE_INSENSITIVE | Pattern.DOTALL)
						.matcher(line);
				if (m.find())
					bandwidth = m.group(1);
				...
				m = Pattern
						.compile(
								"<span id=\"stats-views\" class=\"stat\">([\\d,]*)</span> views",
								Pattern.CASE_INSENSITIVE | Pattern.DOTALL)
						.matcher(line);
				if (m.find())
					viewcount = m.group(1);
