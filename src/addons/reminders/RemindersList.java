...
public class RemindersList
{
	...
	/**
	 * This gets all the events at the current time, it also removes the
	 * events from the list so that they do not repeat.
	...
	public Reminders[] getReminders(Date timeOfEvent)
	{
		...
		{
			/* If the time of the event has passed or is current then we add
			 * it to the list to be returned, we also delete it from are 
			 * reminders list, if we delete a element we don't want to increment
			 * the loop counter as it will skip the next element of the list, 
			 * so we check the current index again and if that isn't a reminder
			 * for the current time then we increment the loop counter.
			 */
		...
		/* Finally convert are ArrayList to an 
		 * array so it can't be manipulated outside this class.
		 */
